...
- **Toolbar popup** (React + Tailwind + Radix): choose **Classic editor**, **New designer**, or **Paused**. Your choice is stored in `chrome.storage.sync` under `enforcedV3` as `"true"`, `"false"`, or `"off"` and applied in the service worker, content script, and DNR layer. If Chrome sync is off, `chrome.storage.sync` still works for that profile; it behaves like local storage ([Chrome `storage` docs](https://developer.chrome.com/docs/extensions/reference/api/storage/)). **Popup appearance** (About tab: System / Light / Dark) is stored separately in **`chrome.storage.local`** under `popupThemePreference` so it stays on this device and does not sync across signed-in browsers.
- While enforcing, adds or replaces the `v3` query parameter so it matches your selected boolean mode.
- Optional **survey flag** (**Survey links** in the popup): **Leave survey links alone (default)** never adds or rewrites `v3survey`. **Turn on survey flag** adds `v3survey=true` when missing and aligns existing values to `true`. Stored as `chrome.storage.sync` key `v3surveyEnabled` (`"true"` / `"false"`).
...
| [`tests/constants.test.ts`](./tests/constants.test.ts)                                 | `parseEnforcementPreference`, `needsDefaultEnforcedV3Seed`, storage keys / defaults, DNR ruleset id strings, and a **manifest drift guard** (MV3 permissions, hosts, `manifest.json`, DNR JSON, icons). |
...
| [`tests/reload-focused-target-tab.test.ts`](./tests/reload-focused-target-tab.test.ts) | Stubbed `chrome.tabs.query` / `reload`: last-focused window, skip when paused or URL not a flow/run target (used after editor and survey saves).                                                        |
| [`tests/theme-preference.test.ts`](./tests/theme-preference.test.ts)                   | `parseThemePreference`, `resolveIsDark`, `prefersDarkFromSystem`, `applyThemeClassToDocument`, `subscribePrefersColorScheme`.                                                                           |
...
| [`src/constants.ts`](./src/constants.ts)                                                                                                | Storage keys (`SYNC_POLICY_KEYS`, `STORAGE_KEY_POPUP_THEME`), ruleset ids, parsers (`parseEnforcementPreference`, `parseV3SurveyEnabled`), install seeds, defaults.                                                                                              |
...
| [`src/popup/`](./src/popup/)                                                                                                            | React popup: Editor / Survey / About tabs; sync-backed enforcement and survey toggles; **local-only** popup theme (`theme-preference.ts`); optional reload of the last-focused flow/run tab after saves when enforcement is on (skipped while Paused).           |
...
- **Survey links:** default leaves URLs alone. Optional **on** adds `v3survey=true` when missing; see the popup **Survey links** section.
...
- **Popup:** switching Classic / New designer saves right away and may refresh the flow or run tab you had focused. Changing **Survey links** saves immediately; the active flow/run tab may reload only when an editor mode is active (not while Paused). **About → Appearance** saves to this device only (`chrome.storage.local`). Background and content still react when `enforcedV3` or `v3surveyEnabled` changes in sync storage.
