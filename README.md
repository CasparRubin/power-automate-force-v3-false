...
**Public summary** (matches manifest `description`): Allows you to enforce either the Classic or New Designer experience in Microsoft Power Automate Cloud Flows using `v3=false` or `v3=true`, while also giving you the option to hide the Microsoft survey prompt asking why you made your selection.
...
- **Helvety monorepo:** keep `public/manifest.json` `description` identical to `POWER_AUTOMATE_EDITOR_ENFORCER_PUBLIC_SUMMARY` in [`helvety/packages/shared/src/power-automate-editor-enforcer-copy.ts`](https://github.com/CasparRubin/helvety/blob/main/packages/shared/src/power-automate-editor-enforcer-copy.ts). The store card / `llms` line uses `POWER_AUTOMATE_EDITOR_ENFORCER_STORE_SHORT_DESCRIPTION` in the same file (summary + Survey/Paused suffix). [`tests/expected-manifest-description.ts`](./tests/expected-manifest-description.ts) mirrors the summary for extension-only CI.
...
- **Save feedback (Editor / Survey):** phased status text and a spinner while `chrome.storage.sync` runs; all tab triggers and the Editor / Survey radio groups stay disabled until the write finishes and `reloadFocusedTargetTabIfApplicable` completes (it may `chrome.tabs.reload` the last-focused **flow/run** tab when enforcement is not **Paused**—see `src/popup/reload-focused-target-tab.ts`, which returns whether a reload was actually invoked).
...
| [`tests/reload-focused-target-tab.test.ts`](./tests/reload-focused-target-tab.test.ts) | Stubbed `chrome.tabs.query` / `reload`: last-focused window, `true`/`false` return when reload ran or was skipped (paused, non-target URL, missing tab, reload error); used after editor and Survey-tab saves.                                                                                                     |
...
| [`src/popup/`](./src/popup/)                                                                                                            | React popup: Editor / Survey / About tabs; sync-backed enforcement and Survey (`v3surveyEnabled`); **local-only** theme (`theme-preference.ts`, `theme-boot.ts` at bundle entry before `createRoot` for CSP-safe first paint); Editor/Survey **save progress** (status, spinner, disabled tabs/radios) and optional last-focused **flow/run** tab reload via `reload-focused-target-tab.ts` (boolean = reload ran). `about-meta.ts` exports public URLs and `EXTENSION_DISPLAY_NAME` (must match manifest `name`). |
...
- **Popup policy saves** (`App.tsx`): after `chrome.storage.sync.set` for `enforcedV3` or `v3surveyEnabled`, the popup awaits `reloadFocusedTargetTabIfApplicable` before showing **Saved.**; the helper’s **boolean** return is used for status auto-dismiss timing (longer only when `chrome.tabs.reload` actually ran). Pending dismiss timers are cleared when a new save starts or the popup unmounts.
...
2. Tag from the repo root (annotated tag matches the manifest line, with a `v` prefix):
   ...
   ```bash
   git tag -a vX.Y.Z -m "Release vX.Y.Z"   # e.g. manifest "2.5.0" → tag v2.5.0
